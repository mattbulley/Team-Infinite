...
function createExchange(id, xLocation, yLocation, capacity) {
  if (!isExchangeIdValid(id)) {
    return {
        success: false,
  ...
  if (!isExchangeCapacityValid(capacity)) {
    return {
        success: false,
        message: 'Exchange ' + id + ' has capacity is out of range.'
      };
  }
...
function isExchangeCapacityValid(capacity) {
...
function addExchange() {
  ...
  let exchageCapacity = $('#ex1Capacity').val();

  response = createExchange(exchangeId, exchangeX, exchangeY, exchageCapacity);
